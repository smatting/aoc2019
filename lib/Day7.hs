...
runIntCode :: VU.Vector Int -> [Int] -> (Program, Either ErrorMsg [Int])
...
runAmp ::  VU.Vector Int -> Int -> Int -> Either ErrorMsg Int
runAmp prog phase input =
  either Left f (snd (runIntCode prog [phase, input]))
    where
      f [x] = Right x
      f _ = Left (ErrorMsg "Wrong output")

runPhases :: VU.Vector Int -> [Int] -> Either ErrorMsg Int
runPhases prog phases =
  let pipes = fmap (runAmp prog) phases
   in foldr (>=>) return pipes 0

solvePart1 :: VU.Vector Int -> Either ErrorMsg (Maybe Int)
solvePart1 input =
...
main = do
  ...
  -- print $ runPhases intcode [4,3,2,1,0]
  print $ fmap maximumMaybe $ solvePart1 intcode
